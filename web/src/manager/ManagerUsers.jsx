...
export default function ManagerUsers({ users, me }) {
  ...
  async function changeRole(u, newRole) {
    ...
    if (u.role === 'admin' && newRole === 'employee' && adminCount <= 1) { alert(t('mgr.usr.promoteFirst')); return; }
    if (u.id === me.id && newRole === 'employee' && !confirm(t('mgr.usr.confirmSelfDemote'))) return;
  ...
  async function removeUser(u) {
    ...
    if (!confirm(t('mgr.usr.deleteConfirm', { name: u.name }))) return;
    try {
      await profilesApi.softDelete(u.id);
      auditApi.log('Employee removed', u.name);
    } catch (e) { alert(t('mgr.ppl.deleteFail', { e: e.message || e })); }
  }
  ...
  return (
    <>
      <NewUser />
      <div className="card">
        <h2>{t('mgr.tab.users')}</h2>
        <div style={{ overflowX: 'auto' }}>
          <table>
            <thead><tr><th>{t('mgr.ppl.colName')}</th><th>{t('mgr.usr.colEmail')}</th><th>{t('mgr.ppl.colRole')}</th><th>{t('mgr.ppl.colStatus')}</th><th></th></tr></thead>
            <tbody>
              {users.map((u) => {
                ...
                return (
                  <tr key={u.id} style={inactive ? { opacity: 0.55 } : null}>
                    ...
                    <td className="nowrap">
                      ...
                      {u.id !== me.id && <button className="btn-ghost btn-sm" style={{ marginLeft: 6 }} onClick={() => toggleActive(u)}>{inactive ? t('mgr.ppl.activate') : t('mgr.ppl.deactivate')}</button>}
                    </td>
